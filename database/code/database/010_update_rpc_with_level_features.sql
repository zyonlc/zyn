-- =============================================
-- UPDATE RPC FUNCTION TO INCLUDE LEVEL AND FEATURES
-- =============================================
  ...
  CASE destination
    WHEN 'media' THEN
      SELECT
        ...
        mpc.is_premium,
        mpc.status,
        COALESCE(mpc.level, 'All Levels'),
        COALESCE(mpc.features, '[]'::jsonb),
        mpc.created_at,
        mpc.updated_at
    ...
    WHEN 'portfolio' THEN
      SELECT
        ...
        ppc.is_premium,
        ppc.status,
        COALESCE(ppc.level, 'All Levels'),
        COALESCE(ppc.features, '[]'::jsonb),
        ppc.created_at,
        ppc.updated_at
